...
class _HomeScreenState extends State<HomeScreen> {
      ...
      builder: (BuildContext dialogContext) { 
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(child: const Text('Logout', style: TextStyle(color: Colors.red)), onPressed: () => Navigator.of(dialogContext).pop(true)),
          ],
        );
      },
    );
        ...
        bottomNavigationBar: BottomNavigationBar(
          items: <BottomNavigationBarItem>[
            BottomNavigationBarItem(
              icon: SvgPicture.asset('assets/icons/home_icon.svg', width: 24, height: 24,
  ...
  Widget _buildPrescriptionSection() {
    ...
    if (_latestPrescription!.exists) { 
      ...
      if (data['issueDate'] is Timestamp) dateIssuedFormatted = DateFormat('dd MMM, yy').format((data['issueDate'] as Timestamp).toDate());
      List<dynamic> medications = data['medications'] ?? [];
      if (medications.isNotEmpty) {
        String medsSummary = medications.take(2).map((med) => "- ${med['medicineName'] ?? 'N/A'} (${med['dosage'] ?? 'N/A'}) - ${med['frequency'] ?? 'N/A'}").join("\n");
